...
class TeacherMessageController extends CI_Controller
{
    ...
    public function teacher_compose()
    {
        $this->check_login();
        $data = array();
        ...
        if ($_POST) {
            ...
            $message = array(
                ...
                'Date' => date("d/m/y"),
                'SenderTeacherID' => $_SESSION['teacher_username']
            );
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($check && ($type == 1)) {
                    ...
                    $data['success'] = $this->TeacherMessageModel->send_message_to_teacher($message);
                } else if ($check && ($type == 2)) {
                    $message['StudentID'] = $receiver;
                    $data['success'] = $this->TeacherMessageModel->send_message_to_student($message);
        ...
        $this->load->view("mailbox/teacher_compose", $data);
    ...
    public function admin_inbox()
    {
        ...
        $data['messages'] = $this->TeacherMessageModel->get_messages($_SESSION['teacher_username'], "ADMIN");
        $this->load->view('mailbox/teacher_mailbox', $data);
    ...
    public function student_inbox()
    {
        ...
        $data['unread_messages'] = $this->TeacherMessageModel->get_unread_messages();
        $data['unread_count'] = is_array($data['unread_messages']) ? count($data['unread_messages']) : 0;
        $data['messages'] = $this->TeacherMessageModel->get_messages($_SESSION['teacher_username'], "STUDENT");
        $this->load->view('mailbox/teacher_mailbox', $data);
    ...
    public function parent_inbox()
    {
        ...
        $data['unread_messages'] = $this->TeacherMessageModel->get_unread_messages();
        $data['unread_count'] = is_array($data['unread_messages']) ? count($data['unread_messages']) : 0;
        $data['messages'] = $this->TeacherMessageModel->get_messages($_SESSION['teacher_username'], "PARENT");
        $this->load->view('mailbox/teacher_mailbox', $data);
    ...
    public function teacher_inbox()
    {
        ...
        $data['unread_messages'] = $this->TeacherMessageModel->get_unread_messages();
        $data['unread_count'] = is_array($data['unread_messages']) ? count($data['unread_messages']) : 0;
        $data['messages'] = $this->TeacherMessageModel->get_messages($_SESSION['teacher_username'], "TEACHER");
        $this->load->view('mailbox/teacher_mailbox', $data);
    ...
    public function read_mail($id = "")
    {
        ...
        if (isset($id) && is_numeric($id)) {
            ...
            $data['unread_messages'] = $this->TeacherMessageModel->get_unread_messages();
            $data['unread_count'] = is_array($data['unread_messages']) ? count($data['unread_messages']) : 0;
            $data['message'] = $this->TeacherMessageModel->get_message($id);

            if ($data['message'])
                $this->load->view('mailbox/teacher_read_mail', $data);
            else
                redirect(site_url("TeacherMessageController/"));
        } else
            redirect(site_url("TeacherMessageController/"));
    }
