...
class StudentMessageModel extends CI_Model
{
    ...
    {
        $this->db->insert($this::TABLE_ADMIN_MESSAGES, $data);
    ...
    public function get_messages($id, $inbox)
    {
        if ($inbox == "ADMIN") {
            ...
            $this->db->select('*')->from($this::TABLE_STUDENT_MESSAGES)->where($check);
        } elseif ($inbox == "TEACHER") {
            $check = array(
                "StudentID" => $id,
                "SenderTeacherID !=" => '0'
            );
    ...
    public function get_message($id)
    {
        ...
        $this->db->select('*')->from($this::TABLE_STUDENT_MESSAGES)->where($check)->limit(1);
        $messages = $this->db->get();


        if ($messages->num_rows() > 0) {
            $this->db->where($check);
            $this->db->update($this::TABLE_STUDENT_MESSAGES, array(
                "Status" => "1"
            ));
        }
    ...
    public function get_unread_messages()
    {
        ...
        $this->db->select('*')->from($this::TABLE_STUDENT_MESSAGES)->where($check)->limit(4);
        $messages = $this->db->get();
